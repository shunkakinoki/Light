...
type TimelineNetworkSnapshotObjectKey = {
  spaceId: string;
  category?: CategoryType;
  cursor?: string;
};

export const useTimelineNetworkSnapshot = (
  spaceId?: string,
  category?: CategoryType,
  cursor?: string,
) => {
  const timelineNetworkSnapshotFetcher = async (
    key,
    obj: TimelineNetworkSnapshotObjectKey,
  ) => {
    const url = `${LIGHT_API_URL}/api/timeline/network/snapshot/${obj?.spaceId}`;
  ...
  const getKey: SWRInfiniteKeyLoader = (
    ...
    if ((previousPageData && !previousPageData?.timeline) || !spaceId) {
      return null;
    }

    if (pageIndex === 0) {
      return [
        SwrKeys.TIMELINE_NETWORK_SNAPSHOT,
        <TimelineNetworkSnapshotObjectKey>{
          spaceId: spaceId,
          category: category,
          cursor: cursor,
        },
  ...
  const { data, error, isValidating, size, setSize } =
    useSWRInfinite<TimelineData>(getKey, timelineNetworkSnapshotFetcher);
