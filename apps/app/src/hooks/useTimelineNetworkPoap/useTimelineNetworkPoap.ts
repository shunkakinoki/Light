...
type TimelineNetworkPoapObjectKey = {
  eventId: string;
  category?: CategoryType;
  cursor?: string;
};

export const useTimelineNetworkPoap = (
  eventId?: string,
  category?: CategoryType,
  cursor?: string,
) => {
  const timelineNetworkPoapFetcher = async (
    key,
    obj: TimelineNetworkPoapObjectKey,
  ) => {
    const url = `${LIGHT_API_URL}/api/timeline/network/poap/${obj?.eventId}`;
  ...
  const getKey: SWRInfiniteKeyLoader = (
    ...
    if ((previousPageData && !previousPageData?.timeline) || !eventId) {
      return null;
    }

    if (pageIndex === 0) {
      return [
        SwrKeys.TIMELINE_NETWORK_POAP,
        <TimelineNetworkPoapObjectKey>{
          eventId: eventId,
          category: category,
          cursor: cursor,
        },
  ...
  const { data, error, isValidating, size, setSize } =
    useSWRInfinite<TimelineData>(getKey, timelineNetworkPoapFetcher);
