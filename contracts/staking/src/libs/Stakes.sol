...
library Stakes {
  ...
  /**
   * @dev Allocate tokens from the main stack to a SpaceDeployment.
   * @param stake Stake data
  ...
  /**
   * @dev Unallocate tokens from a SpaceDeployment back to the main stack.
   * @param stake Stake data
   ...
   */
  function unlockTokens(Stakes.Indexer storage stake, uint256 _tokens)
    internal
  {
   ...
   */
  function withdrawTokens(Stakes.Indexer storage stake)
    internal
    returns (uint256)
  {
   ...
   * @return Token amount
   */
  function tokensUsed(Stakes.Indexer memory stake)
    internal
    pure
    returns (uint256)
  {
   ...
   * @return Token amount
   */
  function tokensSecureStake(Stakes.Indexer memory stake)
    internal
    pure
    returns (uint256)
  {
   ...
   * @return Token amount
   */
  function tokensAvailable(Stakes.Indexer memory stake)
    internal
    pure
    returns (uint256)
  {
   ...
   * @return Token amount
   */
  function tokensWithdrawable(Stakes.Indexer memory stake)
    internal
    view
    returns (uint256)
  {
    ...
    if (
      stake.tokensLockedUntil == 0 || block.number < stake.tokensLockedUntil
    ) {
      return 0;
    }
